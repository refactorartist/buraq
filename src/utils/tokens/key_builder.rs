...
impl KeyBuilder {
    ...
    pub fn generate_key(&self, algorithm: Algorithm) -> Result<KeyPair> {
        match algorithm {
            ...
            Algorithm::EdDSA => 
                Err(Error::msg("EdDSA key generation is not yet implemented")),
        }
    }
    ...
    pub fn generate_hmac_key(
        ...
        let test_data = b"";
        let signature = hmac_key.sign(test_data)
    ...
    pub fn generate_rsa_key(&self, key_length: RsaKeyLength) -> Result<KeyPair> {
        ...
        // Convert the public key to PEM format
        let public_pem = PKey::public_key_from_pem(&public_key)
            .map_err(|e| Error::msg(format!("Failed to create public key from private key: {}", e)))?
            .public_key_to_pem()
