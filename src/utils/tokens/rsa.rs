...
pub fn generate_rsa_key_pair(
    rsa_key_length: RsaKeyLength,
) -> Result<(RsaPrivateKey, RsaPublicKey), Error> {
    // Generate RSA key pair
    let rsa =
        Rsa::generate(rsa_key_length.as_bits() as u32).context("Failed to generate RSA key")?;

    // Create private key
    let private_key = PKey::from_rsa(rsa).context("Failed to create private key")?;
    ...
    let public_key =
        PKey::public_key_from_pem(&public_key).context("Failed to create public key")?;
...
mod tests {
    ...
    fn test_generate_rsa_key_pair_success() {
        ...
        for &key_length in RsaKeyLength::all() {
            ...
            assert!(
                private_bits >= expected_bits && private_bits < expected_bits + 8,
                "Private key size {} doesn't match expected {}",
                private_bits,
                expected_bits
            );
            assert!(
                public_bits >= expected_bits && public_bits < expected_bits + 8,
                "Public key size {} doesn't match expected {}",
                public_bits,
                expected_bits
            );
