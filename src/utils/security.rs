...
impl SecretsManager {
    ...
    pub fn decrypt(&self, encrypted_text: &str, client_id: &Uuid) -> Result<String, Error> {
        ...
        let encrypted_data = STANDARD.decode(encrypted_text)
    ...
    fn encrypt_data(&self, data: &[u8], key: &[u8], iv: &[u8]) -> Result<Vec<u8>, Error> {
        ...
        while key_stream.len() < data.len() {
            ...
            let hmac_key = HmacKey::new(&current_key, HmacHashFunction::Sha256);
            current_key = hmac_key.sign(&[0u8; 1])
                .map_err(|e| Error::msg(format!("HMAC error: {}", e)))?;
        }
...
mod tests {
    ...
    fn test_different_client_ids() {
        temp_env::with_var("MASTER_ENCRYPTION_KEY", Some("test-master-key-12345"), || {
            ...
            let encrypted = secrets_manager.encrypt(original_text, &client_id_1).unwrap();
